...
function WhoImFollowing({myFollowing}) {
    ...
    const { setViewedUserID, setViewAnotherUser, setUsersTweetList, setViewUsersTweets, setViewUsersUsername, isMyFollowersPage } = useContext(AuthContext);
                ...
                {myFollowing.map((user, index) => {
                    return (<>
                        <div className={styles.user} key={user.userID}>
                            ...
                            <div className={styles.username} onClick={() => viewUserProfile(user.userID)}>{user.username}</div>
                            <div className={styles.stats}>Following: {user.following} Followers: {user.followers}</div>
                            </div>
                            <div className={styles.viewTweets} onClick={() => viewUsersTweets(user.userID, user.username)}>View Tweets</div>
                        </div ></>
                    )
                })}
                </div>
